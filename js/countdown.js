(function(global, factory) {
	...
	function parseDate(source, target) {
		target = target || 'yyyy-mm-dd'; // default format
		var parts = source.match(/(\d+)/g),
			i = 0, fmt = {};
		// extract date-part indexes from the format
		source.replace(/(yyy|dd|mm)/g, function(part) { fmt[part] = i++; });
	...
	var countDown = function(params) {
		...
		if (typeof Date.prototype.toISOString == "function") {
			console.log('ES5 ISO date parsing available');
			...
			console.log('ES5 ISO date inavailable');
			this.params.startTime = this.params.startTime ? parseDate(this.params.startTime) : Date.now();
			this.params.endTime = this.params.endTime ? parseDate(this.params.endTime) : null;
		}
